...
<template>
  <a-layout class="admin-layout" style="min-height: 100%">
    ...
    <a-layout class="admin-layout-main" :style="`height: ${windowHeight}px`" id="nexoraOneMain">
      <!-- 顶部头部信息 -->
      <a-layout-header class="nexora-layout-header">
      ...
      <!--中间内容-->
      <a-layout-content class="admin-layout-content" id="nexoraOneLayoutContent">
      ...
      <!-- footer 版权公司信息 -->
      <a-layout-footer class="nexora-layout-footer" v-show="footerFlag">
        <NexoraFooter />
      </a-layout-footer>
      <!---- 回到顶部 --->
...
<script setup>
  ...
  onMounted(() => {
    if (watermarkFlag.value) {
      watermark.set('nexoraOneLayoutContent', useUserStore().actualName);
    } else {
      watermark.clear();
    }
  });
  ...
  //回到顶部
  const backTopTarget = () => {
    return document.getElementById('nexoraOneMain');
  };
  // ----------------------- keep-alive相关 -----------------------
